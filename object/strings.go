...
func formatStr(format string, options []Object) (string, error) {
	...
	for _, opt := range format {
		...
		if check_val && opt == '}' {
			...
			if err != nil {
				return "", fmt.Errorf("Ulichopeana si NAMBA, jaribu tena: `%s'", vstr)
			}

			oVal, exists := optionsMap[arrv]

			if !exists {
				return "", fmt.Errorf("Nambari ya chaguo unalolitaka %d ni kubwa kuliko ulizopeana (%d)", arrv, opts_len)
			}
	...
	if check_val {
		return "", fmt.Errorf("Haukufunga '{', tuliokota kabla ya kufika mwisho `%s'", val.String())
	}

	for _, v := range optionsMap {
		if !v.val {
			return "", fmt.Errorf("Ulipeana hili chaguo (%s) {%s} lakini haukutumia", v.obj.Inspect(), v.obj.Type())
		}
	}
